...
let adderGenerator numberToAdd = fun x -> x + numberToAdd
...
let add2ThanSquare x = x |> add2 |> square
...
let peekResult: Result<int, string> -> Result<int, string> =
    Result.map (fun n ->
        printfn "%i" n
        n)
...
let safeDivide x y =
    if y = 0 then Error "Cannot divide by zero" else Ok(x / y)

let optionToResult errMsg =
    function
    | Some v -> Ok v
    | None -> Error errMsg
